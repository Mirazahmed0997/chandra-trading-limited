...
class Engine extends CI_Controller
{
	...
	public function render_front_view($data, $content, $header, $footer, $main_content)
	{
		$xcaliver = &get_instance();
		$data['header'] = $xcaliver->load->view($header, $data, TRUE);
		$data['footer'] = $xcaliver->load->view($footer, $data, TRUE);
		$data['main_content'] = $xcaliver->load->view($content, $data, TRUE);
		$xcaliver->load->view($main_content, $data);
	}
